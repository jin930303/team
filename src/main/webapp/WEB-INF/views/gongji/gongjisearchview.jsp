...
<html>
<head>
<link href="./css/gongji/gongjisearchview.css" rel="stylesheet"/>
<meta charset="UTF-8">
<title>
</title>
</head>
	...
	<main class="main-container">
			<c:choose>
				<c:when test="${empty list}">
					<div class="title">
						<h1>검색 내용 : " ${svalue} "</h1>
						<div class="ascdesc">
							최신 <a href="gongjidayup"><strong>▲</strong></a><a href="gongjidaydown"><strong>▼</strong></a> 
		...
		<table>	
			...
			<c:choose>
			    <c:when test="${empty list}">
					<tr>
						<td colspan="4">"${svalue}"로 검색한 결과가 없습니다.</td>
					</tr>
				</c:when>
				<c:otherwise>	
...
<!-- 페이징 -->
		<div class="pagination">
		   <c:if test="${paging.startPage!=1 }">
		      <a href="gongjisearchview?nowPage=${paging.startPage-1 }&cntPerPage=${paging.cntPerPage}"></a>
		   </c:if>   
		      <c:forEach begin="${paging.startPage }" end="${paging.endPage}" var="p"> 
		         <c:choose>
		            ...
		            <c:when test="${p != paging.nowPage }">
		               <a href="gongjisearchview?nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a>
		            </c:when>   
		         </c:choose>
		      </c:forEach>
		      
		      <c:if test="${paging.endPage != paging.lastPage}">
		      <a href="gongjisearchview?nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage }">  </a>
		   </c:if>
		</div>
